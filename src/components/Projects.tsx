...
const Projects = () => {
  ...
  const ProjectCard = ({ project, delay = 0 }: { project: typeof projects[0]; delay?: number }) => (
    <div className={`group card-gradient rounded-2xl border border-border/50 hover:border-primary/50 transition-smooth overflow-hidden animate-fadeInUp ${project.featured ? 'lg:col-span-2' : ''}`}
      ...
      <div className="p-8">
        <div className="flex items-start justify-between mb-6">
          <div>
            ...
            {project.featured && (
              <span className="ml-2 text-yellow-500 text-sm font-medium bg-yellow-500/10 px-3 py-1 rounded-full">
                Featured
              </span>
            )}
          </div>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {project.tech.map((tech, index) => (
            <span 
              key={index}
              className="text-sm bg-muted text-muted-foreground px-3 py-1 rounded-full hover:bg-primary hover:text-primary-foreground transition-smooth cursor-default"
            >
              {tech}
            </span>
          ))}
